...
#include "../pathtracing/rt_volumefx.h"

#include <string.h> // memcpy

// External CVars
extern cvar_t *r_useSceneGraph;
cvar_t *r_lodBias;
...
void R_AddDrawSurf( surfaceType_t *surface, shader_t *shader, 
	...
	ds = &tr.refdef.drawSurfs[index];

	// Clear the destination so any extended metadata defaults to zero
	Com_Memset( ds, 0, sizeof( drawSurf_t ) );

	// the sort data is packed into a single 32 bit value so it can be
	// compared quickly during the qsorting process
	ds->sort = (shader->sortedIndex << QSORT_SHADERNUM_SHIFT) 
		| tr.shiftedEntityNum | ( fogIndex << QSORT_FOGNUM_SHIFT ) | (int)dlightMap;
	ds->surface = surface;
	ds->shader = shader;
	ds->fogIndex = fogIndex;
	ds->dlightBits = dlightMap;
	ds->lightMask = dlightMap;
	ds->materialID = shader ? shader->sortedIndex : 0;
	ds->meshID = 0;
	ds->numIndexes = 0;
	ds->firstIndex = 0;
	ds->firstVertex = 0;
	// default transform/color
	ds->modelMatrix[0] = ds->modelMatrix[5] = ds->modelMatrix[10] = ds->modelMatrix[15] = 1.0f;
	ds->normalMatrix[0] = ds->normalMatrix[5] = ds->normalMatrix[10] = ds->normalMatrix[15] = 1.0f;
	ds->color[0] = ds->color[1] = ds->color[2] = 1.0f;
	ds->color[3] = 1.0f;
	ds->bounds[0][0] = ds->bounds[0][1] = ds->bounds[0][2] = -65536.0f;
	ds->bounds[1][0] = ds->bounds[1][1] = ds->bounds[1][2] = 65536.0f;
...
static void R_AddEntitySurfaces( void ) {
	...
	for ( tr.currentEntityNum = 0;
		...
		// Add entity to scene graph if enabled
		if ( r_useSceneGraph && r_useSceneGraph->integer ) {
			node = R_AddEntityToSceneGraph( ent );
			if ( node ) {
				// Set node transform with the entity's position and orientation
				vec3_t angles = {0.0f, 0.0f, 0.0f};
				vec3_t scale = {1.0f, 1.0f, 1.0f}; // Default scale
				
				// TODO: Convert entity axis to angles properly
				// For now using default rotation
				
				R_SetNodeTransform( node, ent->e.origin, angles, scale );
				// Update node transform
				R_UpdateNodeTransform( node );
			}
		}
#ifdef USE_LEGACY_DLIGHTS
		ent->needDlights = 0;
		...
		// Weapon explosion and smoke effects become path-traced volumes in
		// the primary view (portal/mirror views keep the raster sprites, as
		// the path tracer only renders the main view).
		if ( tr.viewParms.portalView == PV_NONE && ent->e.customShader &&
		     ( ent->e.reType == RT_SPRITE || ent->e.reType == RT_MODEL ) &&
		     RT_VolumeFX_InterceptEntity( ent ) ) {
			continue;
		}
...
static void R_GenerateDrawSurfs( void ) {
	// Use portal-based visibility if available
	if ( R_UsePortalCulling() ) {
		// Setup portal frustum for culling
		R_SetupPortalFrustum();
		
		// Add world surfaces using portal visibility
		R_AddWorldSurfacesPortal();
	} else {
		// Fall back to traditional PVS-based visibility
		R_AddWorldSurfaces();
	}
...
void R_RenderView( const viewParms_t *parms ) {
	...
	// Update the path tracer's dynamic light buffer with the current view
	RT_UpdateDynamicLights();
}
