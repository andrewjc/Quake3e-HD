...
#include "../postprocessing/tr_ssr.h"
#include "../lighting/tr_light.h"
#include "../effects/tr_volumetric_explosions.h"
#include "../core/memory/tr_resource.h"
#include "../core/memory/tr_memory.h"
#include "../scene/tr_portal.h"
#include "../scene/tr_scene_graph.h"
#include "../advanced/tr_gpu_driven.h"
#include "../postprocessing/tr_taa.h"
#include "../postprocessing/tr_hdr.h"
#include "../postprocessing/tr_volumetric.h"
#include "../postprocessing/tr_postprocess.h"
#include "../core/command/tr_cmdbuf.h"
#include "../core/profiling/tr_profile.h"
#include "../core/profiling/tr_stats.h"
#include "../core/profiling/tr_perf.h"
#include "../core/profiling/tr_debug.h"
#include "../optimization/tr_optimize.h"
...
cvar_t	*r_neatsky;
cvar_t	*r_drawSun;
cvar_t  *r_modernLighting;
static cvar_t *r_warnLegacyLightmaps;
static int legacyLightmapWarningCount = 0;

cvar_t *r_vertexLight;

void R_ReportLegacyLightmapUsage(const char *context) {
	if (!r_warnLegacyLightmaps || r_warnLegacyLightmaps->integer <= 0) {
		return;
	}

	const char *label = (context && context[0]) ? context : "legacy lightmap path";

	if (r_warnLegacyLightmaps->integer > 1) {
		ri.Error(ERR_FATAL, "Legacy lightmap path used (%s)\n", label);
		return;
	}

	if (legacyLightmapWarningCount < 16) {
		ri.Printf(PRINT_WARNING, "Legacy lightmap path touched: %s\n", label);
	} else if (legacyLightmapWarningCount == 16) {
		ri.Printf(PRINT_WARNING, "Further legacy lightmap warnings suppressed (set r_warnLegacyLightmaps 2 to halt).\n");
	}

	legacyLightmapWarningCount++;
}
#ifdef USE_PMLIGHT
cvar_t	*r_dlightMode;
...
// Portal visibility system CVars
cvar_t	*r_usePortalCulling;
cvar_t	*r_portalOnly;
cvar_t	*r_portalBias;
cvar_t	*r_showPortals;
cvar_t	*r_useSceneGraph;
cvar_t	*r_sceneGraphDebug;
...
void R_EnforceModernLighting( void ) {
	...
	if ( r_modernLighting->integer != 1 ) {
		ri.Cvar_Set( "r_modernLighting", "1" );
	}

	if ( !( r_modernLighting->flags & CVAR_ROM ) ) {
		r_modernLighting->flags |= CVAR_ROM;
	}

	if ( !logged ) {
		ri.Printf( PRINT_DEVELOPER, "Modern lighting pipeline enforced; legacy lighting toggles have been retired.\n" );
		logged = qtrue;
	}
}
...
static void R_Register( void )
{
	...
	r_modernLighting = ri.Cvar_Get( "r_modernLighting", "1", CVAR_ROM );
	ri.Cvar_SetDescription( r_modernLighting, "Read-only: the unified modern lighting pipeline is always enabled, replacing legacy lighting CVars." );
	ri.Cvar_SetGroup( r_modernLighting, CVG_RENDERER );

	// Legacy compatibility shim: keep r_vertexLight defined so old configs don't crash.
	r_vertexLight = ri.Cvar_Get( "r_vertexLight", "0", CVAR_ROM );
	ri.Cvar_SetDescription( r_vertexLight, "Read-only compatibility shim. Vertex lighting mode has been removed; this cvar always resolves to 0." );
	ri.Cvar_SetGroup( r_vertexLight, CVG_RENDERER );

	r_warnLegacyLightmaps = ri.Cvar_Get("r_warnLegacyLightmaps", "1", CVAR_ARCHIVE_ND);
	ri.Cvar_CheckRange(r_warnLegacyLightmaps, "0", "2", CV_INTEGER);
	ri.Cvar_SetDescription(r_warnLegacyLightmaps, "Controls handling of legacy lightmap paths.\n"
		"0: No warnings.\n"
		"1: Print a warning the first few times legacy lightmap code is touched.\n"
		"2: Abort with an error when legacy lightmap code is touched.");
	...
	ri.Cvar_SetDescription( r_showCommandBuffer, "Display command buffer statistics." );
	...
	// Modern renderer debug cvars
	r_showLightVolumes = ri.Cvar_Get( "r_showLightVolumes", "1", CVAR_CHEAT );
	ri.Cvar_SetDescription( r_showLightVolumes, "Debugging tool: Show dynamic light bounding volumes." );
	r_showShadowVolumes = ri.Cvar_Get( "r_showShadowVolumes", "1", CVAR_CHEAT );
	...
	// Path tracing CVARs
	r_rt_enabled = ri.Cvar_Get( "r_rt_enabled", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_enabled, "Enable path traced lighting (0=off, 1=on)." );
	r_rt_quality = ri.Cvar_Get( "r_rt_quality", "2", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_quality, "Path tracing quality (0=off, 1=low, 2=medium, 3=high, 4=ultra)." );
	r_rt_bounces = ri.Cvar_Get( "r_rt_bounces", "3", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_bounces, "Maximum light bounces for path tracing (1-10)." );
	r_rt_samples = ri.Cvar_Get( "r_rt_samples", "2", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_samples, "Samples per pixel per frame (1-16)." );
	r_rt_denoise = ri.Cvar_Get( "r_rt_denoise", "1", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_denoise, "Enable denoising filter (0=off, 1=on)." );
	r_rt_temporal = ri.Cvar_Get( "r_rt_temporal", "1", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_temporal, "Enable temporal accumulation (0=off, 1=on)." );
	r_rt_probes = ri.Cvar_Get( "r_rt_probes", "1", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_probes, "Enable irradiance probes (0=off, 1=on)." );
	r_rt_cache = ri.Cvar_Get( "r_rt_cache", "1", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_cache, "Enable light cache (0=off, 1=on)." );
	r_rt_debug = ri.Cvar_Get( "r_rt_debug", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription(r_rt_debug, "Path tracing debug mode (0=off, 1=rays, 2=probes, 3=cache)." );
	...
	r_shadows = ri.Cvar_Get( "r_shadows", "4", 0 );
	...
	// Portal visibility system CVars
	r_usePortalCulling = ri.Cvar_Get( "r_usePortalCulling", "1", CVAR_ARCHIVE_ND );
	ri.Cvar_SetDescription( r_usePortalCulling, "Enable portal-based visibility culling system." );
	ri.Cvar_SetGroup( r_usePortalCulling, CVG_RENDERER );

	r_portalOnly = ri.Cvar_Get( "r_portalOnly", "0", CVAR_CHEAT );
	ri.Cvar_SetDescription( r_portalOnly, "Only render portal debug information." );
	ri.Cvar_SetGroup( r_portalOnly, CVG_RENDERER );

	r_portalBias = ri.Cvar_Get( "r_portalBias", "0.05", CVAR_ARCHIVE_ND );
	ri.Cvar_SetDescription( r_portalBias, "Portal plane distance bias for visibility calculations." );
	ri.Cvar_SetGroup( r_portalBias, CVG_RENDERER );

	r_showPortals = ri.Cvar_Get( "r_showPortals", "0", CVAR_CHEAT );
	ri.Cvar_SetDescription( r_showPortals, "Debug visualization of portal areas and connections." );
	ri.Cvar_SetGroup( r_showPortals, CVG_RENDERER );

	r_useSceneGraph = ri.Cvar_Get( "r_useSceneGraph", "1", CVAR_ARCHIVE_ND );
	ri.Cvar_SetDescription( r_useSceneGraph, "Enable hierarchical scene graph for spatial queries." );
	ri.Cvar_SetGroup( r_useSceneGraph, CVG_RENDERER );

	r_sceneGraphDebug = ri.Cvar_Get( "r_sceneGraphDebug", "0", CVAR_CHEAT );
	ri.Cvar_SetDescription( r_sceneGraphDebug, "Debug visualization of scene graph nodes and bounds." );
	ri.Cvar_SetGroup( r_sceneGraphDebug, CVG_RENDERER );

	r_bloom_threshold = ri.Cvar_Get( "r_bloom_threshold", "0.3", CVAR_ARCHIVE_ND );
	ri.Cvar_SetDescription( r_bloom_threshold, "Color level to extract to bloom texture, default is 0.3." );
	...
	ri.Cvar_SetDescription( r_bloom_intensity, "Final bloom blend factor, default is 0.3." );
	ri.Cvar_SetGroup( r_bloom_intensity, CVG_RENDERER );

	r_bloom_modulate = ri.Cvar_Get( "r_bloom_modulate", "1", CVAR_ARCHIVE_ND );
	...
	// GPU-driven rendering CVars
	r_gpuDriven = ri.Cvar_Get( "r_gpuDriven", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription( r_gpuDriven, "Enable GPU-driven rendering features." );
	r_gpuCulling = ri.Cvar_Get( "r_gpuCulling", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription( r_gpuCulling, "Enable GPU-based frustum culling." );
	r_meshShading = ri.Cvar_Get( "r_meshShading", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription( r_meshShading, "Enable mesh shading pipeline." );
	r_variableRateShading = ri.Cvar_Get( "r_variableRateShading", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription( r_variableRateShading, "Enable variable rate shading." );
	r_bindlessTextures = ri.Cvar_Get( "r_bindlessTextures", "0", CVAR_ARCHIVE );
	ri.Cvar_SetDescription( r_bindlessTextures, "Enable bindless texture resources." );
...
void R_Init( void ) {
	...
	// Initialize command buffer system for multi-threaded rendering
	R_InitCommandBuffers();
	
	// Initialize GPU resource management
	R_InitResourcePool();
	
	// Initialize portal visibility system (Phase 2)
	R_InitPortalSystem();
	
	// Initialize scene graph (Phase 2)
	R_InitSceneGraph();
	
	// Initialize lighting CVars
	R_InitLightingCVars();
	
	// Initialize GPU-driven rendering (Phase 3)
	R_InitGPUDriven();
	
	// Initialize advanced rendering features (Phase 4)
	R_InitTAA();
	R_InitSSR();
	R_InitVolumetric();
	R_InitHDR();
	...
	// Initialize volumetric explosion system
	R_InitVolumetricExplosions();
	
	// Create render thread if SMP is enabled
	...
	// Initialize Performance Optimization and Profiling (Phase 5)
	ri.Printf(PRINT_ALL, "^2[INIT] Initializing Phase 5 systems...\n");
	R_InitProfiling();
	R_InitRenderStats();
	R_InitPerformanceMonitoring();
	R_InitFrameTiming();
	R_InitMemoryTracking(vk.physical_device, vk.device);
	R_InitRenderOptimization();
	R_InitQueryManager(vk.device, vk.physical_device);
	R_InitDebugVisualization();
	
	// Initialize path tracing system (manages optional RTX backend internally)
	ri.Printf(PRINT_ALL, "^2[INIT] Initializing path tracing system...\n");
	RT_InitPathTracer();
...
static void RE_Shutdown( refShutdownCode_t code ) {
	...
	// Shutdown GPU-driven rendering (Phase 3)
	R_ShutdownGPUDriven();
	
	// Shutdown scene graph (Phase 2)
	R_ShutdownSceneGraph();
	
	// Shutdown portal system (Phase 2)
	R_ShutdownPortalSystem();
	
	// Shutdown resource pool
	R_ShutdownResourcePool();
	
	// Shutdown command buffers
