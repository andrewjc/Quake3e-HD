...
#define TCGEN_MAX (TCGEN_VECTOR + 1)
...
typedef struct {
	...
	unsigned int	isClampMap : 1;
	unsigned int	isLightmap : 1;
...
typedef enum {
	SHADER_CATEGORY_WORLD = 0,
	SHADER_CATEGORY_MODEL,
	SHADER_CATEGORY_UI,
	SHADER_CATEGORY_FX,
	SHADER_CATEGORY_SYSTEM
} shaderCategory_t;
...
typedef struct shader_s {
	...
	int			shaderIndex;			// original shader index (for models, etc.)
	...
	material_t	*material;				// backing material definition (if available)
	...
	int			vertexAttribs;			// required vertex attributes for this shader
...
typedef struct {
	...
	// Extended FOV tracking for weapon/UI handling
	float		weaponFovX;
	float		weaponFovY;
	float		aspectCorrectedFovX;
	float		aspectCorrectedFovY;
...
typedef struct drawSurf_s {
	...
	shader_t			*shader;		// material/shader reference
	trRefEntity_t		*entity;		// owning entity
	int					fogIndex;		// fog volume index
	int					dlightBits;		// dynamic light bitmask
	int					lightMask;		// extended lighting mask
	int					numIndexes;		// index count for this draw
	int					firstIndex;		// index buffer offset
	int					firstVertex;	// vertex buffer offset
	int					materialID;		// material identifier for GPU paths
	int					meshID;			// mesh identifier for GPU paths
	float				modelMatrix[16];// object to world transform
	float				normalMatrix[16];// normal transform
	vec4_t				color;			// per-surface color override
	vec3_t				bounds[2];		// surface bounds for culling
	qboolean			isDynamic;		// dynamic surface? (moving)
	struct interaction_s *firstInteraction; // linked list of light interactions
} drawSurf_t;
...
typedef struct msurface_s {
	...
	vec3_t				bounds[2];		// cached world-space bounds for lighting/culling
	surfaceCullInfo_t	cullinfo;		// additional culling metadata
	qboolean			isDynamic;		// tracked for moving world geometry
	struct interaction_s *firstInteraction; // per-surface light interaction chain
} msurface_t;
...
typedef struct {
	...
	qboolean doneBloom;
	qboolean donePathTracer;    // path-traced output already composited this frame
...
#endif
	vec3_t					sunLight;			// directional light RGB * intensity
	float					sunLightIntensity;	// cached directional light intensity
	renderLight_t			*sunRenderLight;	// persistent render light for sun
	vec3_t					sunDirection;
...
extern cvar_t	*r_gpuDriven;
extern cvar_t	*r_gpuCulling;
extern cvar_t	*r_meshShading;
extern cvar_t	*r_variableRateShading;
...
#define GLS_ALPHATEST_ENABLE					GLS_ATEST_BITS
